...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val syncManager = com.example.data.sync.SyncManager(dao, viewModelScope)
    val geminiAdvisor = com.example.data.ai.GeminiOverhaulAdvisor()
    ...
    // --- Cloud Sync & Local-First State ---
    val syncStatus = syncManager.syncStatus

    // --- Gemini AI Industrial Advisor States ---
    private val _aiBottleneckAnalysis = MutableStateFlow<com.example.data.ai.CriticalBottleneckAnalysis?>(null)
    val aiBottleneckAnalysis: StateFlow<com.example.data.ai.CriticalBottleneckAnalysis?> = _aiBottleneckAnalysis.asStateFlow()

    private val _hseRiskAssessment = MutableStateFlow<com.example.data.ai.HseRiskAssessment?>(null)
    val hseRiskAssessment: StateFlow<com.example.data.ai.HseRiskAssessment?> = _hseRiskAssessment.asStateFlow()

    private val _executiveBriefing = MutableStateFlow<com.example.data.ai.ExecutiveOverhaulBriefing?>(null)
    val executiveBriefing: StateFlow<com.example.data.ai.ExecutiveOverhaulBriefing?> = _executiveBriefing.asStateFlow()

    fun triggerCloudSync() {
        syncManager.triggerSyncNow(_currentUser.value)
    }

    fun runAiBottleneckAnalysis() {
        viewModelScope.launch {
            val items = rawOversightItems.value
            val progress = dashboardKpis.value.overallProgressPercent
            _aiBottleneckAnalysis.value = geminiAdvisor.analyzeCriticalPathAndBottlenecks(items, progress)
        }
    }

    fun runHseRiskAssessment(permitType: String, equipment: String, location: String) {
        viewModelScope.launch {
            _hseRiskAssessment.value = geminiAdvisor.assessHseRisksForPermit(permitType, equipment, location)
        }
    }

    fun generateExecutiveBriefing() {
        viewModelScope.launch {
            val kpis = dashboardKpis.value
            val permits = safetyPermits.value
            _executiveBriefing.value = geminiAdvisor.generateDailyExecutiveBriefing(
                overallProgress = kpis.overallProgressPercent,
                totalTasks = kpis.totalItems,
                completedCount = kpis.completedCount,
                activePermitsCount = permits.count { it.status == "issued" },
                pendingProcurementsCount = kpis.pendingProcurementsCount
            )
        }
    }
