...
interface OverhaulDao {
    ...
    // --- Oversight Items (WBS) ---
    @Query("SELECT * FROM oversight_items WHERE oversightId = :oversightId AND isDeleted = 0 ORDER BY wbsCode ASC, id ASC")
    ...
    @Query("SELECT * FROM oversight_items WHERE oversightId = :oversightId AND executiveUnit = :unit AND isDeleted = 0 ORDER BY wbsCode ASC, id ASC")
    fun getItemsByUnit(oversightId: Long, unit: String): Flow<List<OversightItemEntity>>

    @Query("SELECT * FROM oversight_items WHERE id = :id AND isDeleted = 0 LIMIT 1")
    suspend fun getItemById(id: Long): OversightItemEntity?

    @Query("SELECT * FROM oversight_items WHERE id IN (:ids) AND isDeleted = 0")
    ...
    @Query("SELECT * FROM oversight_items WHERE parentItemId = :parentId AND isDeleted = 0 ORDER BY wbsCode ASC, id ASC")
    ...
    @Query("SELECT * FROM oversight_items WHERE oversightId = :oversightId AND (parentItemId IS NULL OR parentItemId = 0) AND isDeleted = 0 ORDER BY wbsCode ASC, id ASC")
    ...
    @Query("UPDATE oversight_items SET isDeleted = 1, deletedAt = :deletedAt WHERE id = :id OR parentItemId = :id")
    suspend fun softDeleteItemAndDirectSubtasks(id: Long, deletedAt: String)

    @Query("UPDATE oversight_items SET isDeleted = 1, deletedAt = :deletedAt WHERE id = :id")
    suspend fun softDeleteItemById(id: Long, deletedAt: String)
    ...
    // ==========================================
    // 13. CLOUD SYNC QUEUE (LOCAL-FIRST)
    // ==========================================
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertSyncQueueItem(item: SyncQueueEntity): Long

    @Query("SELECT * FROM sync_queue WHERE syncStatus = 'pending' ORDER BY id ASC")
    fun getPendingSyncQueue(): Flow<List<SyncQueueEntity>>

    @Query("SELECT * FROM sync_queue WHERE syncStatus = 'pending' ORDER BY id ASC")
    suspend fun getPendingSyncQueueDirect(): List<SyncQueueEntity>
    ...
    suspend fun updateSyncQueueStatus(id: Long, status: String, error: String? = null)
